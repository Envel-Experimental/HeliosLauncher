            ...
            const checkManifest = (label, url) => {
                ...
                test(`${label} should be accessible and signed`, async () => {
                    try {
                        ...
                        console.warn(`[SKIP] Mirror ${mirror.name} (${label}) unreachable: ${e.message}`);
                    }
            ...
            if (mirror.assets) {
                test('Assets endpoint should be reachable', async () => {
                    try {
                        ...
                        console.warn(`[SKIP] Mirror ${mirror.name} assets unreachable: ${e.message}`);
                    }
                });
            }
            ...
            if (mirror.libraries) {
                test('Libraries endpoint should be reachable', async () => {
                    try {
                        ...
                        console.warn(`[SKIP] Mirror ${mirror.name} libraries unreachable: ${e.message}`);
                    }
                });
            }
        });
