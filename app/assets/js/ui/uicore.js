 ...
 */
// Requirements
Object.assign(window, require('@core/dom_utils'))
const { ipcRenderer, shell, webFrame } = require('electron')

// Remote replacement via HeliosAPI bridge
export const isDev = require('@core/isdev')
const { LoggerUtil } = require('@core/util/LoggerUtil')
export const Lang = require('@core/langloader')
...
// Use HeliosAPI exposed by preload or renderer-entry
export var currentWindow = window.currentWindow || window.HeliosAPI?.window
export var remoteApp = window.HeliosAPI?.app
export var appVersion = window.appVersion || (typeof window.HeliosAPI?.app?.getVersion === 'function' ? window.HeliosAPI.app.getVersion() : '0.0.1')
export var currentWebContents = {
    on: (evt, cb) => ipcRenderer.on(`wc:${evt}`, cb),
    removeListener: (evt, cb) => ipcRenderer.removeListener(`wc:${evt}`, cb)
}
export var remoteDialog = {
...
export let updateCheckListener
/**
    ...
    switch (arg) {
        ...
        case 'update-available':
            ...
            if (process.platform === 'darwin') {
                info.darwindownload = `https://github.com/Envel-Experimental/Flauncher/releases/download/v${info.version}/Foxford-Launcher-setup-${info.version}${process.arch === 'arm64' ? '-arm64' : '-x64'}.dmg`
                showUpdateUI(info)
            }
 ...
 */
export function changeAllowPrerelease(val) {
    ipcRenderer.send('autoUpdateAction', 'allowPrereleaseChange', val)
}

export function showUpdateUI(info) {
    setOverlayContent(
...
export function populateAccountListings() {
...
// System Warnings
export let warningQueue = []

export function showNextWarning() {
